...
| `/open`                              | Браузер каталогов и выбор папки проекта |
| `/ls`                                | Файлы и папки текущего проекта          |
...
`/open` просматривает файловую систему бота и позволяет выбрать папку как текущий
проект. Доступ ограничен переменной `OPEN_BROWSER_ROOTS` (список корней через запятую);
по умолчанию используется домашняя директория пользователя процесса. `/ls` показывает
содержимое выбранного проекта. На VPS это файловая система VPS, а не Windows-ноутбука.
